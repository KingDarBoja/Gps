...
//        Connection connection = null;
        ...
        if (connection != null) {
            ...
            try {
                ...
                writeQueryNewTable(connection,lat,lon,tim);
            } catch (SQLException e) {
                e.printStackTrace();
            }
        } else {
    ...
    private static void writeQueryNewTable(Connection conn, String lat, String lon, String time) {
        Statement statement = null;
        try {
            // STEP 4: Execute a test query on database
            ...
            sql = "INSERT INTO " + TABLE_VEHICLE_2 + " (" + KEY_LAT + "," + KEY_LON + "," + KEY_TIME + ") " + "VALUES " + "('" + lat + "', '" + lon + "', '" + time + "')";
            ResultSet rs = statement.executeQuery(sql);
